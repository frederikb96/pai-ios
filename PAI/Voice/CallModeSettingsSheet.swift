...
/// running call.
///
/// Wraps `SettingsScreen` unmodified rather than building a call-only subset: it needs its own
/// `NavigationStack` for a title bar and a Close button, since it is not reached through
...
/// the target can never drift apart.
