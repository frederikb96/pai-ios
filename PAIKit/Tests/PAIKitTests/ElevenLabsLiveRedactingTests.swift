...
/// Runs unconditionally, with no live flag and no key — this is what proves the redacting helper
/// itself works, independent of whether the live class ever runs.
final class ElevenLabsLiveRedactingTests: XCTestCase {
    func testRedactsATokenQueryParameterInAConnectionURL() {
        ...
        let redacted = ElevenLabsLiveRedacting.redact(url)
    ...
    func testRedactsAnApiKeyHeaderValueHoweverItWasFormatted() {
        ...
        let redacted = ElevenLabsLiveRedacting.redact(dumped)
    ...
    func testLeavesTextWithNoCredentialShapeUntouched() {
        ...
        XCTAssertEqual(ElevenLabsLiveRedacting.redact(text), text)
    }
}
